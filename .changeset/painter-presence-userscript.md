...
- Claim a region with a new drawing tool, from the shapes button on the rail, M for a rectangle, or L for an ellipse; polygons and stars are in its toolbar. Drag anywhere on the map while the map keeps panning and zooming; the tool closes once the claim is saved. Without a tool selected, click one of your claims for handles or drag it to move it, and Delete removes it. Claims are whole pixels with no anti-aliasing and do not need a template underneath.
