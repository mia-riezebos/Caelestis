...
- [x] A report records evidence, limitations, and dispositions for each optional experiment.
...
- [x] Record world/alliance validation and experiment dispositions, then run final checks.
...
- Mia deferred live performance acceptance because 8–10 simultaneous tasks share this machine and Chromium. Timing samples from this session are not accepted performance evidence.
- World matrix and live alliance acceptance remain open under #267. `docs/performance-profiling.md` records the manual checklist and evidence requirements for conditional experiments.
- Browser cleanup discarded temporary native drafts, restored the owned tab to 100% zoom, and stopped CDP focus emulation. No public paint was submitted; tab settings writes were isolated.
- Final validation passed all 1,154 userscript tests, typecheck, production build, repository lint, and 32 release tests. Rebase onto origin/main completed without conflicts.
- After rebase, 53 focused tests passed with one worker. A redundant parallel suite hit timeouts and was stopped; the affected persistence/palette suites passed 38 tests with one worker and a 30-second timeout.
