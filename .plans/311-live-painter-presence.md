...
- Theme key: L had been dead because Wplace moved "Dark mode" from the button's `aria-label` into its text, inside the settings dialog. `wplace-theme.ts` now clicks a visible control, else opens the settings dialog under a hiding stylesheet and clicks it there, else sets `data-theme` plus the `theme` storage key and swaps the map style URL (`maps.wplace.live/styles/liberty|fiord`). Verified on the live page over CDP: N flips `custom-winter` → `dark` → back with no dialog or stylesheet left behind.
