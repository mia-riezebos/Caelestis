...
## TODOs
- [ ] Extend the shared menu model with item groups, checkable items, and a submenu; render separators and a hover/keyboard/touch submenu in TemplateTree with tests.
- [ ] Rebuild the userscript menu entries as ordered groups shared by template, folder, and server rows; update tests.
- [ ] Replace the lifecycle rows with a "Mark as…" submenu carrying Finished and Frozen with checked state; update tests.
