<script lang="ts">
  ...
  let { progress, size = 'md', showPercent = true, griefWatch = false, class: className = '' }: {
...
<div class={`meter-wrap ${className}`}>
  ...
  {#if showPercent}<span class:alarm-text={griefWatch && progress.mismatched > 0} class:small={size === 'sm'} class="percent">{percent >= 99.95 && percent < 100 ? '99.9' : (Math.round(percent * 10) / 10).toFixed(1)}%</span>{/if}
</div>

<style>
  .meter-wrap { display: flex; min-inline-size: 0; align-items: center; gap: 0.5rem; }
  .track { display: flex; flex: 1; min-inline-size: 0; block-size: 0.75rem; overflow: hidden; border: 1px solid var(--caelestis-border); border-radius: 999px; background: var(--caelestis-raised-surface); }
  .track.small { block-size: 0.5rem; }
  .track span { block-size: 100%; }
  .completed { background: var(--caelestis-success); }
  ...
  .percent.small { font-size: 0.72rem; }
