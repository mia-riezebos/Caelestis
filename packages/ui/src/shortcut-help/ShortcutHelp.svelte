  ...
  function shortcutSetsFor(platform: ShortcutHelpPlatform): readonly ShortcutSet[] {
    ...
    return [
      ...
      },
      {
        ...
        description: 'M enters claim mode with the rectangle tool. Inside, the keys follow Illustrator: V selection (drag empty canvas for a marquee; rotate from the grip or just outside a corner), A direct selection (anchors on every shape), Q lasso, P pen, N pencil (pixels), B paintbrush, E eraser, M rectangle, L ellipse, H hand or hold Space to pan; scroll pans, Alt/Option or Ctrl/Cmd+scroll zooms; Enter saves your claims, Delete removes the selection, Escape leaves. Hold or right-click a drawer button for its other tools.',
...
<style>
  ...
  .caelestis-keymap-key { display: inline-flex; min-inline-size: 0; block-size: 3.5em; flex: 0 0 var(--caelestis-key-basis); align-items: center; justify-content: center; padding: 0 0.25rem; border: 1px solid var(--caelestis-border); border-radius: var(--caelestis-radius, calc(0.7rem + 1px)); background: var(--caelestis-surface); box-shadow: 0 1px 0 var(--caelestis-border), inset 0 -1px 0 color-mix(in oklab, var(--caelestis-text) 8%, transparent); color: var(--caelestis-text); font: 700 1em/1 ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, monospace; opacity: 0.3; transition: opacity var(--tt-in-dur) ease-out, transform var(--tt-in-dur) ease-out; }
