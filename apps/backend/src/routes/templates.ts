...
import { NodeNotFoundError, TemplateOverlapError } from '../ports/index.js'
...
export const createTemplateRoutes = (ports: Pick<Ports, 'blobs' | 'sql'>, auth: AuthOptions) => {
  ...
  routes.post('/', async (c) => {
    ...
    try {
      ...
      // 409: the upload is well-formed and the group is real, but the placement collides with a
      // template already there. Retrying it unchanged will not help; moving it will.
      if (error instanceof TemplateOverlapError) {
