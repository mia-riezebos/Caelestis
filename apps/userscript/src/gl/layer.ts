...
/**
 * A template's current opacity, for anything drawn alongside it.
 *
 * The markers live in their own layer — above wplace's draft pixels, where the overlay itself must
 * not go — and still have to arrive and leave with the template they belong to.
 */
...
export const installOverlayLayer = (): boolean => {
  ...
  try {
    ...
    // Markers go in a second layer, above the templates and above wplace's draft pixels. The
    // overlay belongs *under* a pixel waiting to be placed; an annotation about that pixel does not.
    map.addLayer(markerLayer, before)
    log(
      'install',
