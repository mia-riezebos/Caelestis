...
vi.mock('../ui/confirm.js', () => ({ confirmDestructive: notices.confirmDestructive }))
vi.mock('../ui/notification-host.js', () => ({ showOneTimeSecret: notices.showOneTimeSecret }))
vi.mock('../ui/toast.js', () => ({ toast: notices.toast }))
...
describe('access-token controller', () => {
  it('supersedes an older first-page request after creating a token', async () => {
    ...
    const { accessTokensModel, createServerAccessToken, refreshAccessTokens } = await import(
      './access-tokens.js'
    )
  ...
  it('waits for a background first-page refresh before loading its next page', async () => {
    ...
    const { accessTokensModel, loadMoreAccessTokens, refreshAccessTokens } = await import(
      './access-tokens.js'
    )
    accessTokensModel(server, changed)
  ...
  it('does not share an in-flight request with replacement credentials', async () => {
    ...
    await vi.waitFor(() =>
      expect(accessTokensModel(replacement, changed).tokens.map((item) => item.label)).toEqual([
        'New credential',
      ]),
    )
  ...
  it('supersedes an in-flight later page with a post-create refresh', async () => {
    ...
    state.listAccessTokens
      ...
      .mockResolvedValueOnce({
        tokens: [token('Fresh', '2'.repeat(64))],
        nextCursor: refreshedCursor,
      })
    const { accessTokensModel, createServerAccessToken, loadMoreAccessTokens } = await import(
      './access-tokens.js'
    )
    accessTokensModel(server, changed)
    await vi.waitFor(async () => expect(await labels()).toEqual(['Initial']))
  ...
  it('exposes creation errors and confirms revocation before mutating', async () => {
    ...
    state.createAccessToken.mockResolvedValueOnce({
      ok: false,
      message: 'Label already exists',
    } as never)
    const { accessTokensModel, createServerAccessToken, revokeServerAccessToken } = await import(
      './access-tokens.js'
    )
    accessTokensModel(server, changed)
    ...
    await vi.waitFor(() =>
      expect(accessTokensModel(server, changed).createError).toBe('Label already exists'),
    )
    ...
    expect(notices.confirmDestructive).toHaveBeenCalledWith(
      expect.objectContaining({ body: 'Painter will stop working immediately.' }),
    )
  })
})
