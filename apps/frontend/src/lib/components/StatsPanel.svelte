<script lang="ts">
  ...
  $effect(() => {
    ...
    contributions = null
    leaderboard = null
    // The heatmap draws up to twenty-six weeks, and that is all this read is for: painter pace comes from
    // the bucket ladder below, at whatever range the scope has.
    const contributionsFrom = Math.floor(Date.now() / 1_000) - 86_400 * 7 * 26
    if (liveDashboard)
    ...
    const refresh = (): void => {
      ...
      void Promise.all([
        getContributions(ids, requestedAt - 86_400 * 7 * 26, requestedAt).then((response) => {
